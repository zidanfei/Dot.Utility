            ...
            if (method.IsDefined(typeof(CachingAttribute), false))
            {
                ...
                switch (cachingAttribute.Method)
                {
                    case CachingMethod.Get:
                        try
                        {
                            if (CacheManager.Cache.Exists(key, valKey))
                            {
                                var obj = CacheManager.Cache.Get(key, valKey);
                            ...
                            else
                            {
                                ...
                                CacheManager.Cache.Add(key, valKey, methodReturn.ReturnValue);
                                return methodReturn;
                            }
                        }
                        catch (Exception ex)
                    ...
                    //            }
                    //            else
                    //                CacheManager.Cache.Put(key, valKey, methodReturn.ReturnValue);
                    //        }
                    //        else
                    //            CacheManager.Cache.Add(key, valKey, methodReturn.ReturnValue);
                    //        return methodReturn;
                    ...
                    //        {
                    //            if (CacheManager.Cache.Exists(removeKey))
                    //                CacheManager.Cache.Remove(removeKey);
                    //        }
                    //        var methodReturn = getNext().Invoke(input, getNext);
                    //        return methodReturn;
                    //    }
