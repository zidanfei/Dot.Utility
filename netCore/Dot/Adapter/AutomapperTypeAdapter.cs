...
namespace Dot.Adapter
{
    ...
    public class AutomapperTypeAdapter : ITypeAdapter
    {
        ...
        {
            foreach (var config in MapperConfigurationList)
            {
                var tempMap = config.GetAllTypeMaps().FirstOrDefault(pre => pre.SourceType == type);
                if (tempMap != null)
                    return tempMap.DestinationType;
            }
            return null;
        ...
        internal List<MapperConfiguration> MapperConfigurationList = new List<MapperConfiguration>();
