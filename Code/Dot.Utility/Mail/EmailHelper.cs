...
namespace Dot.Utility.Mail
{
    ...
    public class EmailHelper : IEmailHelper
    {
        ...
        void ValidateParameters()
        {
            ...
            {
                throw new ConfigException("没有发送邮箱，可以通过配置项 EmailFrom 设置！");
            }
            if (string.IsNullOrEmpty(Host))
            {
                throw new ConfigException("没有 SMTP 事务的主机的名称或 IP 地址，可以通过配置项 EmailHost 设置！");
            }
            if (string.IsNullOrEmpty(To))
            {
                throw new ConfigException("没有收件人！");
            }
            if (string.IsNullOrEmpty(Subject))
            {
                throw new ConfigException("没有标题！");
            }
